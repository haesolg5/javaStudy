...
//		one.privateMethod();
		...
		/*
		private로 선언된 멤버에 외부클래스에서 접근하려면 default 혹은 public으로 선언되는
		외부접근이 가능한 메서드를 통해 간접적으로 접근해야한다. 클래스 내부에서는 접근지정자의 영향을
		...
		/*
		동일한 패키지에 선언된 클래스이므로 import 없이 인스턴스 생성이 가능하다. 또한 default 클래스는
		동일 패키지내에서 접근할 수 있다.
		 */
...
//		two.defaultMethod();
		 ...
		 */
		System.out.println("DefaultClaa 객체 생성 및 접근");
