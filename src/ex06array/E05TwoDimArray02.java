...
public class E05TwoDimArray02 {
	...
	public static void main(String[] args) {
		...
		2. 난수를 이용하여 배열을 초기화한다. (0~100까지)
		2. 배열전체요소를 매개변수로 전달된 정수만큼 증가시킨다.
		4. 증가되기 전, 후 배열을 출력한다.
		5. 다음 메소드명으로 구현한다. twoDimPlus()
		 */
		// 2행 4열 크기의 2차원배열을 생성
		int[][] arrRandom = new int[2][4];
		...
		for(int i=0; i<arrRandom.length; i++) {
			for(int j=0; j<arrRandom[i].length; j++) {
				// 난수를 통해 각 원소 초기화
				arrRandom[i][j] = random.nextInt(101);
			}
		}
