...
/*
배열 : 순차적인 메모리의 공간이 할당되는 형태로서 하나가 아니라 
	여러개의 변수가 필요한 경우 사용
...
public class E01OneDimArray01 {
	...
	public static void main(String[] args) {
		...
		/*
		배열의 선언과 동시에 초기화하는 경우 자바 컴파일러는 초깃값의 갯수를 통해
		배열의 크기를 결정할 수 있다. 따라서 이런 겨우에는 크기를 생략하도록 약속되어있다.
		 */
		System.out.println("방법 2: 선언과 동시에 초기화");
		//int[] initArr2 = new int[3] {1, 20, 300}; // 잘못된 선언
		int[] initArray2 = new int[] {1, 20, 300};
		for(int j = 0; j < initArray2.length; j++) {
			System.out.println(initArray2[j]);
		}
		
		/*
		배열을 원소만으로 생성한 후 초기화까지 진행한다. 이 경우 new를 생략할 수 있다.
		 */
