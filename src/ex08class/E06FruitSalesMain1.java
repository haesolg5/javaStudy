...
class FruitSeller{
	...
	/*
	멤버변수의 경우 초깃삾이 없는 상태로 정의한 후 인스턴스 생성 후 초기화할 수 있다.
	하지만 멤버상수는인스턴스 생성과 상관없이 정의하는 시점에 반드시 초기화해야한다.
	여기서 1000을 지우면 에러가 발생한다.
	 */
...
class FruitBuyer{
	...
	2. 판매자가 금액에 해당하는 사과의 갯수를 반환하면, 그만큼 보유한 사과가 증가한다. 
		=> numOfApple += 메서드호출()
