...
public class PersonConstructor {
	...
	// 생성자 메소드
	/*
	디폴트 생성자 : 해당 클래스에 생성자를 정의하지 않으면 아래와 같이 매개변수, 실행부가 없는 생성자가
	컴파일러에 의해 자동으로 삽입된다. 지금까지 new 클래스명() 과 같이 사용했던 문장이 바로 
	...
	public PersonConstructor(String name) {
		/*
		this는 멤버변수와 매개변수를 구분하기 위한 요도로 사용된다.
		클래스 자신, 즉 멤버를 가리킨
		 */
		this.name = name;
		age = 1;
	...
	void initialize(String name, int age, String addr) {
		/*
		이 코드는 생성자 내에서만 사용할 수 있다. 생성자는 인스턴스 생성시 딱 한번만 호출되고
		개발자가 임의로 호출할 수 없다.
