...
-Array라는 이름처럼 '배열'의 특성을 가지고있어 index를 통해
저장 및 접근이 가능하다.
*/
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		ArratList와나머지 컬렉션들은 데이터를 저장하는 내부적인
...
//		list.add(5, "소방차ㅡㅡ"); //에러발생됨
		...
		저장할 수 있다. 동일한 데이터라도 index로 구분할 수 있기 때문이다.
		...
		/*
		츨력하기
		방법1 : 일반 for문 사용. 이대는 인덱스를 통해 접근해야
			하므로 get() 메서드를 사용한다. */
		System.out.println("\n[일반 for문 사용]");
		...
		/*
		방법2 : 확장 for문의 경우 인덱스없이 전체 원소를 반복헤주므로
		...
		/*
		Iterator 사용법
		1. 컬렉션에 저장된 내용을 Iterator 에게 알려주기 위해
			인스턴스를 생성한다.
		...
		/*
		방법2 : 방법1과 동일하게 인덱스로 삭제한다. 단
		indexOf로 인스턴스의 인덱스를 찾은 후 이 값을 이용한다. 아르르르*/
		...
		방법3 : 인스턴스의 참조값을 통해 삭제한다. 이 경우에는
			삭제에 성공할 경우 true가 반환된다. 즉 boolean 타입의
			반환값을 가진다. */
		//삭제 성공
		System.out.println(list.remove("방탄소년단") ?
				"삭제성공(방탄)":"삭제실패(방탄)");
		//"dhakdlrjf"인스턴스가 없으므로 삭제 실패
