...
//과일 상자 : Object기반으로 모든 과일(인스턴스)을 저장할 수 있는 상자
class FruitBox{
	//멤버변수 : 모든 인스턴스 저장 가능
	Object item;
	//셍성자, getter, setter 정읭
...
public class Ex01GenericBasic {
	...
	public static void main(String[] args) {
		...
		orange1.showInfo();
		/*
		상자1은 오렌지 전용박스이므로 다른과일(즉 인스턴스)를 저장하라 수 없다.
		컴파일 에러가 발생하므로 실행자체가 불가능하다.
		즉 자료형에는 안전하지만, 구현에는 불편함이있다.
		다른 인스턴스를 저장하기 위해서는 또 다른 Box클래스는 생성해야한다. */
		
		/*
		아래와 같이 Orange가 아닌 인스턴스를 저장하면 즉시 컴파일에서가
		...
		/*
		Object를 기반으로 생성된 FriutBox는 구현에는 편리하나
		...
		/*
		하지만 오렌지같은 String을 저장하고, 이를 꺼내서 다운캐스팅하는 과정에서
		에러가 발생된다. 즉 런타임ㅇ에러가 발생하게 되므로 실행전에는 오류를 
		예측하기 힘든 단점이 있다. */
