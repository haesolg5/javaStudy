...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		/*
		아래 2개의 컬렉셔은 내부적인 자료구조만 다를 뿐 구현방법에는
		차이가 없다. 인스턴스 생성 시 Student형의 타입매개변수를 통해 생성한다.
		Student를 저장할 수 있는 상자가 된다. */
		LinkedList<Student> list2 = new LinkedList<Student>();
		//ArrayList<Student> list2 = new ArrayList<Student>();
		
		/*
		저장할 Stident 인스턴스 생성. 외부패키지에 선언되었으므로
		import 후 사용할 수 있다. */
		...
		list2.add(st3);
		list2.add(st4); //인덱스3
		list2.add(st2); //List는 인덱스로 구분할 수 있으므로 '중복저장" 가능ㅇㅇㅇㅇ
		...
		for(Student st : list2) {
			/*
			이름으로 검색 시 멤버변수는name은 private으로 선언되었으므로
			getter를 통해 접근해야한다. 삭제를 위해 입력한 이름과
			Student인스턴스에 저장된 name과 일치하는지 확인한 후
			해당 인스턴스를 검색할 수 있다.*/
