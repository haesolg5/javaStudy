...
public class E04OverrideApply {
	...
	public static void main(String[] args) {
		/*
		err을 사용하면 콘솔에 빨간색으로 표시된다. 주로 디버깅용으로 사용하거나 출력문장을
		구분할 때 사용한다.
		 */
		...
		따라서 오버라이딩 메서드를 호출할 때는 참조변수의 영향을 받지 않는다.
		 */
		child.excecise();
		child.sleep();
		child.printPatent();
		...
		child.walk();
		// 자식쪽에 정의된 메서드로 오버로딩이 정의됨
		child.walk(25);
		/*
		정적메서드는 클래스명을 통해 호출한다. static의 기본 규칙을 따르고
		오버라이딩의 대상이 될 수 없다.
		 */
		...
		두 클래스가 상속관계에 있기 때문에 가능하다. (서로 다른 타입)
		 */
		...
		/*
		부모클래스의 참조변수로 자식인스턴스를 참조하면 부모의 영역까지만 접근할 수 있으므로 자식영역에
		정의된 메서드는 호출할 수 없다.
		 */
		//parent.study();  // error
		parent.walk();
		//parent.walk(20); // error
