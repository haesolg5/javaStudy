...
public class E01JunsuType {
	...
	-각 자료형의 표현범위는 교안을 참조
	-일반적으로 CPU는 정수형 연산을 가장 빠르게 처리하는 특징이 있다. 
	 즉, int에 최적화 되어있다.
	...
	public static void main(String[] args) {
		...
		/*
		100/200의 결과는 0
		Java에서는 정수와 정수를 연산하면 정수의 결과 반환
		실수의 결과를 반환받고 싶다면 형변환(Type casting)을 해야한다. 
		 */
		result1 = int1 / int2;
		System.out.println("int1 / int2 = " + result1);
		
		/*
		-동일한 타입의 변수를 2개 이상 선언하는 경우에는 , 를 사용할 수 있다.
		...
		-큰 자료형과 작은 자료형을 연산하면 큰자료형으로 자동 형변환된다.
		  -> 데이터의 손실(오차)이 있을 수 있기 때문이다.
		-단, byte와 short는 예외적인 상황으로 생각한다. 
		...
		/*
		자바는 정수형 상수를 기본적으로 int형으로 간주하기 때문에 아래 정수를 메모리에 
		로드하는 순간 에러가 발생한다. 이때 접미사 L 또는 l을 붙여서 int형의 자료가 아닌 
		long형의 자료임을 표시한다.
		이와같이 입력되는 숫자도 자료형을 기반으로 메모리에 로드되는데 
