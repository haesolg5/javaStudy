...
/*
싱글톤 디자인패턴
: 클래스를 설계하는 디자인패턴의 하나로서 하나의 인스턴스, 
  즉 하나의 메모리를 할당한 후 이를 프로그램 전체에서 공유해서 사용할때 쓰는 패턴
  
작성방법
1. 생성자의 접근지정자를 private로 선언
2. 클래스 외부에서는 생성자에 접근할 수 없으므로 new를 통해 인스턴스생성을 할 수 없다.
...
class NoSingleTone {
	...
	클래스 외부 혹은 다른 패키지에서는 호출할 수 없기 때문에 new를 통해 인스턴스 
	생성 자체가 불가능하기 때문이다.
	 */
	public NoSingleTone() {}
}
...
class SingleTone {
	...
	/*
	생성자를 private로 선언하면 클래스 외부에서는 호출할 수 없으므로 new를 통해 인스턴스 생성불가
	 */
	private SingleTone() {}
...
//		SingleTone st1 = new SingleTone(); // error
		...
		/*
		정적메서드이므로 클래스명을 통해 직접 호출할 수 있다. 해당 메서드 호출을 통해 싱글톤 인스턴스의
		참조값을 얻어올 수 있다.
		 */
		// 여기에서 얻어오는 참조값과 ...
