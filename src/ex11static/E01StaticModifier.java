...
- main() 메서드가 실행되기 전에 초기화되므로 main() 메서드 실행시 별도의 선언없이 
  변수를 즉시 사용할 수 있다.
...
- 클래스 외부
	1. 인스턴스 변수를 통해 접근한다.(권장x)
	2. 인스턴스 생성 없이 클래스명으로 직접 접근한다.(권장)
- 메서드 앞에 static을 붙이면 정적메서드가 된다. 정적변수와 모든 특성이 동일하다.
 */
class MyStatic{
	/*
	멤버변수 : 인스턴스형 멤버변수는 반드시 클래스의 인스턴스를 생성한 후참조변수를 통해 접근해야한다.
			즉, 일반적인 멤버변수는 인스턴스를 생성하는 시점에 메모르에 로드된다.
			(멤버변수는 Stack영역에, 인스턴스는 Heap영역에 생성됨)
	 */
	int instanceVar;
	
	/*
	정적변수 : 인스턴스를 생성할 필요없이 클래스명으로 바로 접근할 수 있다. JVM(자바가상머신)이
			프로그램을 시작할때 미리 메모리(Method영역)에 로드된다.
	 */
	...
	/*
	정적 멤버메서드 : 정적메서드 내부에서는 인스턴스형 멤버에는 접근할 수 없다. 정적 멤버만 접근이 가능하다.
				 (※ main 메서드에서는 static으로 선언된 메서드만 호출할 수 있었던 것을 기억하자.)
	 */
	static void staticMethod() {
		//System.out.println("instanceVar= " + instanceVar); // error
		System.out.println("staticVar= " + staticVar);
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		static(정적)	변수 접근방법
		: 클래스명을 통해 직접 접근한다. 인스턴스를 생성하지 않고 바로 사용할 수 있다.
		 */
		...
		/*
		2개의 인스턴스를 생성한다. 메모리(Heap영역)에는 ms1, ms2의 인스턴스가 각각 생성된다.
		 */
		...
		/*
		정적변수는 메모리(Method영역)에 딱 하나만 생성되므로 어떤 인스턴스 변수로 접근하든 동일한
		메모리가 된다. 따라서 값을 공유하게 되므로 동일한 결과를 출력한다.
		 */
		ms1.staticVar = 800;
		ms2.staticVar = 900;
		...
		/*
		클래스명을 통한 접근은 정적메서드만 가능하다. 인스턴스 메서드는 인스턴스 참조변수를 통해서만
		접근할 수 있다.
		 */
//		MyStatic.instanceMethod(); // error
		MyStatic.staticMethod();
		/*
