...
디폴드 생성자는 자동으로 삽입되고, 자식쪽에는 super()도 자동으로 삽입될 수 있으므로
생략 가능한 코드가 존재한다.
 */
...
/*
상속(Inheritance)
: 기존에 정의된 클래스에서 메서드와 변수를 추가하여 새로운 클래스를 정의하는 것을 말한다.
  부모클래스를 자식클래스에 상속할 때 extends를 사용한다.
 */
...
class BusinessMan extends Man {
	...
	// 멤버변수 : 이름 => get메서드로 부모 클래스의 name을 가져온다.
	private String name = getName();
	
	// 생성자 : 멤버변수는 2개지만 매개변수는 3개로 선언
	public BusinessMan(String name, String company, String position) {
		/*
		상속받은 자식클래스는 부모클래스의 생성자를 호출하여 부모인스턴스가 메모리에 전저 생성되도록
		해야한다. super()는 부모클래스의 생성하를 호출해준다. 만약 super()를 기술하지 않으면
		컴파일러에 의해 자동으로 삽입되어 디폴트 생성자를 호출하게 된다.
		 */
	...
	public void tellYourInfo() {
		...
		/*
		부모클래스의 멤버변수 name은 private로 선언되었으므로 상속받은 자식클래스에서 접근할 수 없다.
