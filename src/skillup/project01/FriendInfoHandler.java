...
main메서드는 프로그램의 출발점의 역할만 담당하고 모든 기능은 핸들러 클래스에서 구현한다.
 */
public class FriendInfoHandler {
	
	/*
	연락처를 저장하기 위한 인스턴스형 배열을 생성 고딩/대딩 각각의 배열이 필요하고 카운트를 위한 변수도
	생성한다.
	 */
	...
	public FriendInfoHandler(int num) {
		...
		// 입력된 정보의 갯수(or index) 카운트를 위한 변수
		numOfhighFriend = 0;
...
//		Scanner scan = new Scanner(System.in);
		...
		// 고딩
		for(int i=0; i<numOfhighFriend; i++) {
			// 삭제할 이름과 일치하는 인스턴스를 찾기 위해 equals() 사용
			// 일치할 때 true를 반환
		...
		if(deleteIndexHigh!=-1) {
			...
			for(int i = deleteIndexHigh; i<numOfhighFriend; i++) {
				/*
				삭제된 인스턴스의 인덱스부터 마지막까지 반복하면서 뒷쪽의 인스턴스를 한칸씩
				앞으로 이동한다. 
				배열 가운데 null값이 입력되어 있으면 정보출력시 NullPointerException이 
				발생하기 때문이다.
				 */
				highFriends[i] = highFriends[i+1];
			}
			System.out.println("##고딩데이터("+deleteIndexHigh+"번)가 삭제되었습니다.##\n");
		}
		
		if(deleteIndexUniv!=-1) {
			// 대딩 삭제 => 배열 정리
			for(int i = deleteIndexUniv; i<numOfunivFriend; i++) {
				univFriends[i] = univFriends[i+1];
			}
			System.out.println("##대딩데이터("+deleteIndexUniv+"번)가 삭제되었습니다.##\n");
		}
		
		if(deleteIndexHigh==-1 && deleteIndexUniv==-1) {
			// 모두 -1을 유지한다면 검색된 값이 없다는 의미
			System.out.println("##"+deleteName+"의 데이터를 찾을 수 없습니다.##\n");
		}
	}
